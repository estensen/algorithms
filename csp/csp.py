...
class CSP:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # The assignment with the fewest options, but still more than one
        # because then we can't find a better option
        best_choice = None
        fewest = float('inf')
        for var in assignment:
            if len(assignment[var]) > 1:
                if len(assignment[var]) < fewest:
                    best_choice = var
                    fewest = len(assignment[var])
        return best_choice
    ...
    def revise(self, assignment, i, j):
        ...
        for val in assignment[i]:
            if len(assignment[j]) == 1 and assignment[j][0] == val:
                assignment[i].remove(val)  # Prune the neighbors values
                revised = True
        return revised

    def assignment_complete(self, assignment):
        for val in assignment:
            if len(assignment[val]) != 1:
                return False
        return True
...
if __name__ == '__main__':
    for path in glob.glob("sudokus/*txt"):
        print path
        csp = create_sudoku_csp(path)
        print_sudoku_solution(csp.backtracking_search())
        print "Backtrack calls %i" % csp.backtrack_calls
        print "Backtrack fails %i" % csp.backtrack_fails
        print
        print
